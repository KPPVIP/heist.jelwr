...
function VangelicoSetup()
    ...
    SetEntityHeading(glass, -53.06) -- sta/sz/ek#/96/09
...
function Smash(index)
    ...
    ESX.TriggerServerCallback('vangelicoheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            cam = CreateCam("DEFAULT_ANIMATED_CAMERA", true)
            SetCamActive(cam, true) -- sta/sz/ek#/96/09
...
AddEventHandler('vangelicoheist:client:startGas', function()
    ...
    loadPtfxAsset(ptfxAsset) -- sta/sz/ek#/96/09
...
RegisterNetEvent('vangelicoheist:client:showNotification') -- sta/sz/ek#/96/09
